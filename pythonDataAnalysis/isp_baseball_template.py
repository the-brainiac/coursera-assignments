...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return list(filter(lambda x: x.get(yearid) == str(year), statistics))
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    top_player = sorted(statistics, key=lambda x: formula(info, x), reverse=True)[:numplayers]
    return [(player['player'], formula(info, player)) for player in top_player]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    """
    pid, fname, lname = info['playerid'], info['firstname'], info['lastname']
    list_dict = read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])
    top_dict = []
    for dummy_top in top_ids_and_stats:
        top_dict.append(list(filter(lambda x: x[pid] == dummy_top[0], list_dict))+[dummy_top[1]])
    res = []
    for data in top_dict:
        res.append("{:.3f} --- {} {}".format(data[-1], data[0][fname], data[0][lname]))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    """
    list_dict = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    master_dict = read_csv_as_list_dict(info['masterfile'], info['separator'], info['quote'])
    stat = list(filter(lambda x: x[info['yearid']] == str(year), list_dict))

    top_player = sorted(stat, key=lambda x: formula(info, x), reverse=True)[:numplayers]
    res = []
    pid, fname, lname = info['playerid'], info['firstname'], info['lastname']
    for data in top_player:
        dic = list(filter(lambda x: x[pid] == data[pid], master_dict))
        val = formula(info, data)
        res.append("{:.3f} --- {} {}".format(val, dic[0][fname], dic[0][lname]))
    # print(res)
    return res
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    for data in statistics:
        player_set.add(data[playerid])
    for val in sorted(player_set):
        res[val] = {playerid:val}
        for field in fields:
            res[val][field] = 0
    for field in fields:
        for data in statistics:
            res[data[playerid]][field] += int(data[field])
    return res
def compute_top_stats_career(info, formula, numplayers):
    ...
    players = aggregate_by_player_id(list_dict, info['playerid'], fields) 

    top_player = sorted(players.values(), key=lambda x: formula(info, x), reverse=True)[:numplayers]
    res = []
    pid, fname, lname = info['playerid'], info['firstname'], info['lastname']
    for data in top_player:
        dic = list(filter(lambda x: x[pid] == data[pid], master_dict))
